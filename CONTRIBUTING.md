...
This section covers the most common issues contributors run into during local development.

### `npm run compile` fails

**Symptoms:** TypeScript errors, missing types, or `esbuild` errors printed to the terminal.

**What to check:**

1. **Generated files are missing** — If you see errors about missing types in `src/shared/proto/` or `src/generated/`, the proto stubs have not been generated yet. Run:
   ```bash
   npm run protos
   ```
   Then try `npm run compile` again.

2. **Dependency mismatch** — If you recently pulled changes that updated `package.json`, re-install dependencies:
   ```bash
   npm run install:all
   ```

3. **Stale `.tsbuildinfo`** — Incremental build state can get out of sync. Delete it and recompile:
   ```bash
   find . -name "*.tsbuildinfo" -not -path "*/node_modules/*" -delete
   npm run compile
   ```

4. **Enum or type added to `.proto` but not to the conversion map** — If a proto file was edited, `src/shared/proto-conversions/cline-message.ts` may need a new entry. Check the TypeScript error location and add the missing case.

### What does `npm run protos` do and when should I run it?

`npm run protos` invokes the protobuf compiler (`protoc`) on all `.proto` files under `proto/` and writes generated TypeScript into three directories:

| Directory | Purpose |
|---|---|
| `src/shared/proto/` | TypeScript message types shared between extension and webview |
| `src/generated/grpc-js/` | Low-level gRPC service stubs |
| `src/generated/nice-grpc/` | Promise-based client wrappers used by the webview |

Run `npm run protos` whenever you:

- Add, rename, or delete a `.proto` file
- Add or remove an RPC method from a service
- Add or remove a field from a message
- Add a new enum value (e.g., to `ClineSay` in `ui.proto`)

You do **not** need to commit the files in `src/generated/` or `src/shared/proto/` — they are listed in `.gitignore` and regenerated by each developer (and in CI) from the checked-in `.proto` source files.

### How to test the extension locally

1. Make sure the project compiles: `npm run compile`
2. Open the repository in VS Code
3. Press **F5** to launch an **Extension Development Host** window
4. In that new window, open a project folder and interact with Cline as a user would
5. To pick up code changes without restarting, run `npm run compile` (or use watch mode) and press **Ctrl+R** / **Cmd+R** in the Extension Development Host window to reload

For automated tests:

```bash
# Unit tests (fast, no VS Code process needed)
npm run test

# E2E tests (spins up a real VS Code instance via Playwright)
npm run test:e2e
```

### System prompt snapshot tests fail after a prompt change

After modifying any file under `src/core/prompts/system-prompt/`, regenerate snapshots:

```bash
UPDATE_SNAPSHOTS=true npm run test:unit
```

Review the diff in `src/core/prompts/system-prompt/__tests__/__snapshots__/` and commit the updated snapshots together with your prompt changes.
